...
def call_service(api_service, service_type):
    if api_service == "1" or api_service == "CoinAPI" :
        if service_type == "1" or service_type == "current_price":
            ...
            asset_ticker = input("Inserisci il ticker (es. BTC) : ")
            val_change = input("Inserisci la valuta di riferimento (es. USD) : ")
            period_id = input("Inserisci periodo (es. 1DAY, 1HRS, 30MIN) : ")
            time_start = input("Inserisci data inizio (es. 2023-01-01) : ")
            time_end = input("Inserisci data fine (es. 2023-01-01) : ")
            time_series = serviceCoinAPI.get_time_series(asset_ticker,val_change,period_id,time_start,time_end)
            print(time_series)
...
def main():
    ...
    while True:
        ...
        if scelta == "1":
            ...
            service_type = input("Scegli tra i seguenti (current_price : 1, historical_price : 2, time_series : 3): ")
